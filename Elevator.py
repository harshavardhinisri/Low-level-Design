...
# --- Elevator ---
class Elevator:
    def __init__(self, id: int, max_floor: int):
        self.id = id
...
class Strategy1(ElevatorSelectionStrategy): #the one with the fewest pending requests
    def select_elevator(self, elevators, requested_floor):
        print(f"Strategy1: for floor {requested_floor}")
        # Return the elevator with the fewest requests
        return min(elevators, key=lambda e: len(e.requests))

class Strategy2(ElevatorSelectionStrategy): #round robin
    def __init__(self):
        self.index = 0

    def select_elevator(self, elevators, requested_floor):
        print(f"Strategy2: for floor {requested_floor}")
...
class ElevatorController:
    ...
    def __new__(cls, num_elevators=1, max_floor=10, strategy=None):
        if cls.__instance is None:
            cls.__instance = super().__new__(cls)
            cls.__instance.elevators = [Elevator(i, max_floor) for i in range(num_elevators)]
            cls.__instance.strategy = strategy or Strategy2()
...
if __name__ == "__main__":
    controller = ElevatorController(num_elevators=2, max_floor=10, strategy=Strategy2())
    ...
    controller2 = ElevatorController()
    print("Same controller?", controller is controller2)
